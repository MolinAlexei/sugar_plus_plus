...
def generate_mc_data(N=10000, Mb=-19.2, alphas=[-0.15, 3.8], stds=[1., 0.1],
    ...
    if step is None:
        ...
        return z, mb, data, data_cov, mass, proba, np.array(truth_theta), disp_int

@timer
def test_hubble_diagram():

    z, mb, data, data_cov, mass, proba, truth_theta, truth_disp_int = generate_mc_data(N=3000, Mb=-19.2,
                                                                                       alphas=[-0.15, 3.8],
                                                                                       stds=[1, 0.1],
                                                                                       mb_err=0.03,
                                                                                       stds_err = [0.05, 0.01],
